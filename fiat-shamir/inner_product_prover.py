...
def one_round(g_vec,h_vec,u,P,a_vec,b_vec,transcript,p):
	'''
	implements one round of the inner product proof
	'''
	n = len(g_vec)
	if n==1:
		transcript['a'] = str(a_vec)
		transcript['b'] = str(b_vec)
	...
	if transcript.get('L',None) is not None:
	...
	if transcript.get('IPx',None) is not None:
	...
	return gd,hd,u,Pd,ad,bd,transcript



def prover(transcript,p,g,h,u,P,a,b):
	'''
	...
	while g is not None:
		g,h,u,P,a,b,transcript = one_round(g,h,u,P,a,b,transcript,p)
