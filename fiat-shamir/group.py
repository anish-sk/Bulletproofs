...
def fiatshamir(seed,val):
	rand = 1
	seed = seed.encode()
	target_len = len(bin(val)) - 2
	curr_len = len(bin(rand)) - 2

	while curr_len < target_len:
		hash_c = sha256(seed).digest()
		seed += hash_c
		rand *=bytes_to_long(hash_c)
		curr_len = len(bin(rand)) - 2
...
def SaveTranscript(dictionary,File):
