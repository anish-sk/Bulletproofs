...
#Get V by deserializing the corresponding file
with open('V','rb') as f:
  V = Vector.deserialize(p, f.read(),G)
...
def delta(yn,z):
	...
	tmp2 = tmp2*(z*z*z)*(zm.inner_prod(ones_m))
	return (tmp1 - tmp2)%p

Vz2 = V.exp(zm.mult(z*z))
...
for j in range(1,m+1):
  h_temp = Vector(hdash.v[(j-1)*n:j*n],p)
  tmp3 = twos.mult(pow(z,j+1,p))
  tmp2 = tmp2.mult(h_temp.exp(tmp3))
